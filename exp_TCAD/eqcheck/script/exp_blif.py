...
for name in bench_list:
   ...
   if args.ite:
      log   = cur_path + ('/exp_TCAD/eqcheck/log/%s_blif_i.log' % name)
      tcl   = cur_path + ('/exp_TCAD/eqcheck/tcl/exp_blif_i.tcl')
      blif1 = cur_path + ('/exp_TCAD/eqcheck/blif/%s_1_i.blif' % name) 
      blif2 = cur_path + ('/exp_TCAD/eqcheck/blif/%s_2_i.blif' % name) 
   else:
      log   = cur_path + ('/exp_TCAD/eqcheck/log/%s_blif.log' % name)
      tcl   = cur_path + ('/exp_TCAD/eqcheck/tcl/exp_blif.tcl')
      blif1 = cur_path + ('/exp_TCAD/eqcheck/blif/%s_1.blif' % name) 
      blif2 = cur_path + ('/exp_TCAD/eqcheck/blif/%s_2.blif' % name) 
