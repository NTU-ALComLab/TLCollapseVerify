...
if args.ite:
   csv = open(cur_path + '/exp_TCAD/eqcheck/csv/time_i.csv', 'w')
   csv.write('benchmark,cnf,pb,blif,aig\n')
   for name in bench_list:
      t_c = cnf_get_time(cur_path + ('/exp_TCAD/eqcheck/log/%s_cnf_sol_i.log' % name))
      t_p = pb_get_time(cur_path + ('/exp_TCAD/eqcheck/log/%s_pb_sol_i.log' % name))
      t_b = blif_get_time(cur_path + ('/exp_TCAD/eqcheck/log/%s_blif_i.log' % name))
      t_a = aig_get_time(cur_path + ('/exp_TCAD/eqcheck/log/%s_aig_i.log' % name))
      write_time(csv, name, t_c, t_p, t_b, t_a)
else:
   csv = open(cur_path + '/exp_TCAD/eqcheck/csv/time.csv', 'w')
   csv.write('benchmark,cnf,pb,blif,aig\n')
   for name in bench_list:
      t_c = cnf_get_time(cur_path + ('/exp_TCAD/eqcheck/log/%s_cnf_sol.log' % name))
      t_p = pb_get_time(cur_path + ('/exp_TCAD/eqcheck/log/%s_pb_sol.log' % name))
      t_b = blif_get_time(cur_path + ('/exp_TCAD/eqcheck/log/%s_blif.log' % name))
      t_a = aig_get_time(cur_path + ('/exp_TCAD/eqcheck/log/%s_aig.log' % name))
      write_time(csv, name, t_c, t_p, t_b, t_a)
csv.close()
